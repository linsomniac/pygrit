...
def test_diff_status_matches_git(diff_repo: Path) -> None:
    ...
    while i < len(fields):
        ...
        i += 2
    repo = pygritlib.Repository.discover(str(diff_repo))
...
def test_diffstat_with_gitlink_matches_git(gitlink_repo: Path) -> None:
    ...
    repo = pygritlib.Repository.discover(str(gitlink_repo))
...
def test_diffstat_propagates_read_error(
    ...
    a = pygritlib.ObjectId.from_hex(empty_tree)
    b = pygritlib.ObjectId.from_hex(bad_tree)
