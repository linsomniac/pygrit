...
def test_commit_fields_match_git(simple_repo: Path) -> None:
    ...
    commit = repo.commit(pygritlib.ObjectId.from_hex(oid))
...
def test_tz_offset_parsing_non_utc(tmp_path: Path, git_env: dict[str, str]) -> None:
    ...
    pyrepo = pygritlib.Repository.discover(str(repo))
    commit = pyrepo.commit(pygritlib.ObjectId.from_hex(oid))
...
def test_commit_multiparent_parents(tmp_path: Path, git_env: dict[str, str]) -> None:
    ...
    pyrepo = pygritlib.Repository.discover(str(repo))
    commit = pyrepo.commit(pygritlib.ObjectId.from_hex(merge_oid))
...
def test_blob_data_matches_git(simple_repo: Path) -> None:
    ...
    repo = pygritlib.Repository.discover(str(simple_repo))
    blob = repo.blob(pygritlib.ObjectId.from_hex(blob_oid))
...
def test_blob_on_non_blob_raises(simple_repo: Path) -> None:
    ...
    repo = pygritlib.Repository.discover(str(simple_repo))
    tree_oid = rev_parse(simple_repo, "HEAD^{tree}")
    with pytest.raises(pygritlib.InvalidObjectError):
        repo.blob(pygritlib.ObjectId.from_hex(tree_oid))
...
def test_typed_accessors_verify_object_kind(simple_repo: Path) -> None:
    ...
    repo = pygritlib.Repository.discover(str(simple_repo))
    blob_oid = pygritlib.ObjectId.from_hex(rev_parse(simple_repo, "HEAD:a.txt"))
    tree_oid = pygritlib.ObjectId.from_hex(rev_parse(simple_repo, "HEAD^{tree}"))
    commit_oid = pygritlib.ObjectId.from_hex(rev_parse(simple_repo, "HEAD"))
...
def test_typed_accessors_reject_blob_with_parseable_payload(
    ...
    pyrepo = pygritlib.Repository.discover(str(repo_dir))
    # commit() must reject the blob on KIND, even though its bytes parse as a commit.
    with pytest.raises(pygritlib.InvalidObjectError):
        pyrepo.commit(pygritlib.ObjectId.from_hex(blob_oid))
...
def test_tag_fields_match_git(tmp_path: Path, git_env: dict[str, str]) -> None:
    ...
    pyrepo = pygritlib.Repository.discover(str(repo))
    tag = pyrepo.tag(pygritlib.ObjectId.from_hex(tag_oid))
