...
// raw body bytes (see from_bytes). tree/parents are pygritlib ObjectIds.
//
...
// AIDEV-NOTE: ObjectKind is a Python enum.IntEnum defined in python/pygritlib/__init__.py.
...
/// Convert a grit-lib object kind into the public `pygritlib.ObjectKind` IntEnum member.
pub fn kind_to_py(py: Python<'_>, k: grit_lib::objects::ObjectKind) -> PyResult<Py<PyAny>> {
    let cls = OBJECT_KIND_CLS.get_or_try_init(py, || -> PyResult<Py<PyAny>> {
        Ok(py.import("pygritlib")?.getattr("ObjectKind")?.unbind())
    })?;
...
// `frozen` (immutable). `kind` is stored as the already-constructed pygritlib.ObjectKind
// IntEnum member (built once at read time via kind_to_py) so the getter can hand back
// the singleton (identity-comparable: `obj.kind is pygritlib.ObjectKind.BLOB`). `data` is
...
impl TreeEntry {
    ...
    /// The `pygritlib.ObjectKind` derived from the mode (see `mode_to_kind`).
    #[getter]
